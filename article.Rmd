...
housing = read.csv(file = "train.csv")
a1_set = housing %>% select(SalePrice, Neighborhood, GrLivArea) %>% filter( Neighborhood == c("NAmes", "Edwards", "BrkSide")) #pull dataset into R, select paramters for A1.
fit <- lm(SalePrice ~ GrLivArea + Neighborhood + GrLivArea*Neighborhood, data = a1_set) #initial naive model, no transform
summary(fit)

#----------------

fit_transform <- lm(log(SalePrice) ~ log(GrLivArea) + Neighborhood + log(GrLivArea)*Neighborhood, data = a1_set) #transformed model to improve residuals and assumptions
summary(fit_transform)
confint(fit_transform)
