...
For the second analysis in this project, we were tasked to generate 4 models using variations of parameters selected by Forward Selection, Backwards Elimination, Stepwise Selection, and a personalized approach. Using these paramters, we can generate Adjusted R\^2 values, CV Press statistics, and finally submit our model to the Kaggle website to obtain a "Kaggle Score" for each model.
...
To improve the quality of our model, we look at outliers as analaysis shows marginal improvements to R\^2 and marginal deterioration to the Press Statistic. We notice the following outliers:

-   Excessively Expensive \> 700k SalePrice: ID #692 & \# 1183

-   Excessively Large \> 100k sq ft.: ID #314, #226, #256, #707

However, removing them did not improve the model, so we decided to leave them in. 

Next, we removed the following highly correlated variables to reduce multicollinearity: LotFrontage, MasVnrArea, GarageYrBlt, OverallQual, TotRmsAbvGrd, GarageCars, TotalBsmtSF, X1stFlrSF, X2ndFlrSF, FullBath. In the correlation matrix, we can see which plots are correlated. After the elimination of the variables mentioned, we see more fairly spare correlated variables.
...
Mean(SalePrice) = b0 + b1(LotArea) + b2(OverallCond) + b3(YearBuilt) + b4(YearRemodAdd) + b5(BsmtFinSF1) + b6(BsmtFinSF2) + b7(BsmtFinSF3) + b8(GrLivArea) + b9(BsmtFullBath) + b10(KitchenAbvGr) + b11(Fireplaces) + b12(GarageArea) + b13(WoodDeckSF) + b14(EnclosedPorch) + b15(ScreenPorch) + b16(PoolArea) + b17(YrSold)

## Model Results

| Model                | Adjusted R\^2 | RMSE      | CVPress  |
|----------------------|---------------|-----------|----------|
| Forward Selection    | 0.8333        | 0.1620974 | 43.77074 |
| Backward Elimination | 0.8333        | 0.1620974 | 43.77074 |
| Stepwise Selection   | 0.8333        | 0.1620974 | 43.77074 |
